package 
{
	...
	public class Take_Control_Scroller extends EventDispatcher
	{
		...
		public var mc : Take_Control_728_MC;
		public var progress : Number;

		public function Take_Control_Scroller(mc : Take_Control_728_MC)
		{
			this.mc = mc;
			add_event_listeners();
		}
		...
		private function start_drag(event : MouseEvent) : void
		{
			...
			mc.handle.startDrag(false, new Rectangle(0, -1, mc.track.width, 0));
		}

		private function report_handle_pos(event : MouseEvent) : void
		{
			this.progress = (mc.handle.x / mc.track.width );
		...
		{
			TweenLite.to(mc.handle, .2, {x:Math.round(number*mc.track.width)});
		}
	}
}
