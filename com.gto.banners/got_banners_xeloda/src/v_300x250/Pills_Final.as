package v_300x250
{
	...
	{
		public var mc : Pills_Final_MC;
		private var timeline : TimelineLite;

		public function Pills_Final(mc : Pills_Final_MC)
		{
			this.mc = mc;
			init();
			timeline = new TimelineLite();
			timeline.append(TweenLite.to(mc.arrrow_mc, .25, {x:60, scaleX:1.5, scaleY:1.5}));
			timeline.append(TweenLite.to(mc.emails_mc, .25, {scaleX:1.3, scaleY:1.3}),-.25);
			timeline.append(TweenLite.to(mc.arrrow_mc, .25, {x:62}));
			timeline.append(TweenLite.to(mc.arrrow_mc, .25, {x:60}));
			timeline.pause();
		}
