package v_728
{
	...
	{
		private static const MENU_OFFSET : Number = 120;
		...
		private function animate_in() : void
		{
			...
			timeline.append(new TweenLite(all_mc.mc_choose_accordian, 1, {alpha:1, y:-53, ease:Back.easeOut}));
			timeline.append(new TweenLite(all_mc.mc_share_accordian, 1, {alpha:1, y:-53, ease:Back.easeOut}), -.5);
			timeline.append(new TweenLite(all_mc.mc_learn_accordian, 1, {alpha:1, y:-53, ease:Back.easeOut}), -.5);
		}
		...
		{
			share_start = share_accordian.mc.x;
			learn_start = learn_accordian.mc.x;
		}
		...
		private function close_all(event : MouseEvent) : void
		{
			...
			new TweenLite(share_accordian.mc, .25, {delay:.15, x:share_start, ease:Linear.easeNone});
			new TweenLite(learn_accordian.mc, .25, {delay:.15, x:learn_start, ease:Linear.easeNone});
		}
		...
		private function adjust_for_learn(event : MouseEvent = null) : void
		{
			...
			new TweenLite(share_accordian.mc, .25, {delay:.08, x:share_start, ease:Linear.easeNone});
			new TweenLite(learn_accordian.mc, .25, {delay:.16, x:learn_start, ease:Linear.easeNone});
		}
		...
		private function adjust_for_share(event : MouseEvent = null) : void
		{
			...
			new TweenLite(share_accordian.mc, .25, {delay:.15, x:share_start, ease:Linear.easeNone});
			new TweenLite(learn_accordian.mc, .25, {delay:.16, x:learn_start + MENU_OFFSET});
		}
		...
		private function adjust_for_choose(event : MouseEvent = null) : void
		{
			...
			new TweenLite(share_accordian.mc, .25, {delay:.17, x:share_start + MENU_OFFSET});
			new TweenLite(learn_accordian.mc, .25, {delay:.16, x:learn_start + MENU_OFFSET});
		}
